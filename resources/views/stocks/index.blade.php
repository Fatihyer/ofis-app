  ...
  <div class="stock-card">
    <div class="stock-card-header">
      ...
      <form method="GET" action="{{ route('stocks.index') }}" class="filter-row">
        ...
        @php $managedIds=array_map('intval',array_merge($bulkProductIds,$onDemandProductIds));$managedProducts=$products->filter(fn($n,$id)=>in_array((int)$id,$managedIds,true));$otherProducts=$products->reject(fn($n,$id)=>in_array((int)$id,$managedIds,true)); @endphp
        <select name="product_id" class="form-control form-control-sm" onchange="this.form.submit()" style="width:220px"><option value="">Tous les produits</option>@if($managedProducts->isNotEmpty())<optgroup label="Produits suivis">@foreach($managedProducts as $id=>$name)<option value="{{ $id }}" {{ (string)$productId===(string)$id?'selected':'' }}>{{ $name }}</option>@endforeach</optgroup>@endif @if($otherProducts->isNotEmpty())<optgroup label="Autres produits">@foreach($otherProducts as $id=>$name)<option value="{{ $id }}" {{ (string)$productId===(string)$id?'selected':'' }}>{{ $name }}</option>@endforeach</optgroup>@endif</select>
