...
<div class="container-fluid">
  <div class="d-flex align-items-center justify-content-between mb-3">
    <div>
      <h3 class="mb-1">Rapport mensuel — Temps de travail chauffeurs</h3>
      <div class="text-muted">
        Mois : <strong>{{ $ref }}</strong> ({{ $start }} → {{ $end }}) —
        Chauffeurs : {{ $summary['count'] }} —
        Activité Hermes : <strong>{{ $secToHHMM($summary['driving_sec']) }}</strong> — Planning : <strong>{{ $secToHHMM($summary['planning_sec'] ?? 0) }}</strong> — Mission : <strong>{{ $secToHHMM($summary['mission_sec'] ?? 0) }}</strong>
      </div>
    </div>

    <form method="GET" action="{{ route('hermes.drivers.working.monthly') }}" class="d-flex gap-2">
      <input type="month" name="month" value="{{ $ref }}" class="form-control" style="max-width:180px;">
      <button class="btn btn-primary">Afficher</button>
      <a class="btn btn-outline-secondary" href="{{ route('hermes.drivers.working.weekly', ['day' => $start]) }}">Semaine</a>
      <a class="btn btn-outline-secondary" href="{{ route('hermes.drivers.working.daily', ['day' => $start]) }}">Jour</a>
    </form>
  </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped table-hover mb-0 align-middle">
          ...
          <tbody>
            ...
            @empty
              <tr><td colspan="10" class="text-center text-muted py-4">Aucune donnée sur ce mois.</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
